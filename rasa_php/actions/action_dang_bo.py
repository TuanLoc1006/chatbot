...
class ActionDangBo(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        for entity in entities:
            if entity.get('entity') == "dang_bo":
                input_value = entity.get('value')
                print("----"+input_value)
        if input_value in info_dang_bo:
            response = info_dang_bo[input_value]
        else:
...
class ActionDangUy(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        info_dang_uy = {
            ...
            'chi bộ cơ sở':"CHI BỘ CƠ SỞ:</br>- Chi bộ Hành chính</br>- Chi bộ Tổ chức -  Khoa học</br>- Chi bộ Tài chính</br>- Chi bộ Quản trị thiết bị</br>- Chi bộ Đào tạo</br>- Chi bộ Thông tin -  Thư viện</br>- Chi bộ Giáo dục y học</br>- Chi bộ Trung tâm Đào tạo theo nhu cầu</br>- Chi bộ Khoa học cơ bản</br>",
            'đảng bộ cơ sở':"ĐẢNG BỘ CƠ SỞ</br>- Đảng bộ bệnh viện</br>- Đảng bộ Khoa Y</br>- Đảng bộ Chính quy Y</br>- Đảng bộ Liên thông Y</br>- Đảng bộ Răng Hàm Mặt</br>- Đảng bộ Dược</br>- Đảng bộ Y tế công cộng</br>- Đảng bộ Điều dưỡng</br>- Đảng bộ Y học cổ truyền",
            ...
            'nhiệm vụ': "</br>Nhiệm vụ của đảng:</br>Lãnh đạo thực hiện nhiệm vụ chính trị</br>Lãnh đạo thực hiện công tác giáo dục chính trị, tư tưởng</br>Lãnh đạo công tác xây dựng tổ chức đảng và đảng viên</br>Lãnh đạo công tác tổ chức, cán bộ</br>Lãnh đạo và thực hiện công tác kiểm tra, giám sát</br>Lãnh đạo các đoàn thể chính trị - xã hội</br>Lãnh đạo công tác bảo vệ chính trị nội bộ</br>"
        }
        
        input_value = None
        
        for entity in entities:
            if entity.get('entity') == "dang_uy":
                input_value = entity.get('value')
                print("----"+input_value)
        if input_value in info_dang_uy:
            response = info_dang_uy[input_value]
        else:
            response= "Xin lỗi, tôi không có thông tin về câu hỏi của bạn."
        dispatcher.utter_message(text=response)
        return []
