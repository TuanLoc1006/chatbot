...
class ActionProvideDepartmentInfo(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        # đia_diem = next(tracker.get_latest_entity_values('info_type'), None)
        print(department,info_type)
        department_info = {
            "Y": {
                "location": "Tầng 1, Tòa nhà Khoa Y, Trường Đại học Y Dược Cần Thơ",
                "email": "khoay@ctump.edu.vn",
                "phone": "0292. 3830344"
            },
            "Dược": {
                "location": "Tầng 2, Tòa nhà Khoa Răng Hàm Mặt - Dược, Trường Đại học Y Dược Cần Thơ",
                "email": "khoaduoc@ctump.edu.vn",
                "phone": "0292. 3731257"
            },
             "y tế công cộng": {
                "location": "Tầng trệt, Tòa nhà Khoa Y tế công cộng, Trường Đại học Y Dược Cần Thơ",
                "email": "khoaytcc@ctump.edu.vn",
                "phone": "0292. 3730741"
            },
            "điều dưỡng": {
                "location": "Tầng 1, Tòa nhà Khoa Điều Dưỡng - Kỹ thuật y học",
                "email": "khoadieuduong@ctump.edu.vn",
                "phone": "0292. 3897704"
            },
            "kỹ thuật y học": {
                "location": "Tầng 1, Tòa nhà Khoa Điều Dưỡng - Kỹ thuật y học",
                "email": "khoaktyh@ctump.edu.vn",
                "phone": "0292. 3897704"
            },
            "răng hàm mặt": {
                "location": "Tầng 2, Tòa Nhà Răng Hàm Mặt - Dược, Trường Đại học Y Dược Cần Thơ",
                "email": "khoarhm@ctump.edu.vn",
                "phone": "0292. 3731 130"
            },
            "học cơ bản": {
                "location": "Tầng trệt, Tòa Nhà Khoa Khoa học cơ bản, Trường Đại học Y Dược Cần Thơ",
        ...
        if department in department_info:
            location = department_info[department]["location"]
            email = department_info[department]["email"]
            phone = department_info[department]["phone"]

            if info_type == "địa điểm":
                response = f"Địa điểm của {department} là: {location}"
            elif info_type == "email":
                response = f"Email của {department} là: {email}"
            elif info_type == "số điện thoại":
                response = f"Số điện thoại của {department} là: {phone}"
            else:
                response = f"Thông tin về {department}:\nĐịa điểm: {location}\nEmail: {email}\nSố điện thoại: {phone}"
        else:
            response = f"Xin lỗi, tôi không có thông tin về {department}"

        dispatcher.utter_message(text=response)
       
        return [SlotSet("department", None), SlotSet("info_type", None)]
